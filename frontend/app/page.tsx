...
type Stock = { store_id: string; store: string; available: number };
...
type Product = { id: string; sku: string; name: string; brand: string; family: string; customer_price: number; list_price: number; tax_rate: number; total_available: number; stock: Stock[] };
...
function ProductCard({product,onAdd}:{product:Product;onAdd:(id:string,qty?:number)=>void}) { const [qty,setQty]=useState(1); return <article className="product"><div className="product-visual" aria-hidden="true"><span>{product.family.slice(0,2).toUpperCase()}</span><small>{product.family}</small></div><span className="family-name">{product.family}</span><h3>{product.name}</h3><span className="sku">{product.sku} · {product.brand}</span><div className="price">{money(product.customer_price)}</div><span className="small">sin IVA · tarifa {money(product.list_price)}</span><div className="stock">● {Math.max(0,Math.round(product.total_available))} uds. disponibles</div><div className="product-actions"><input className="qty" type="number" min="1" value={qty} onChange={e=>setQty(Math.max(1,Number(e.target.value)))}/><button className="secondary block" onClick={()=>onAdd(product.id,qty)}>Añadir al carrito</button></div></article> }
