...
type OrderLine = { sku: string; description: string; quantity: number; unit: string; unit_price: number; line_total: number };
...
type Order = { id: string; number: string; status: string; workflow_status?: string; workflow?: WorkflowStep[]; store: string; customer: string; created_by: string; customer_reference?: string; job_name?: string; notes?: string; subtotal: number; tax_total: number; total: number; created_at: string; items?: OrderLine[]; documents?: OrderDocument[] };
...
function Home({customer,orders,products,onNavigate,onAdd}:{customer:AccountCustomer|null;orders:Order[];products:Product[];onNavigate:(v:View)=>void;onAdd:(id:string)=>void}) {
  ...
  <div className="home-grid"><div><section className="shortcut-grid"><button onClick={()=>onNavigate("orders")}><span>↻</span><b>Pedidos anteriores</b><small>Consulta y duplica pedidos</small></button><button onClick={()=>onNavigate("documents")}><span>▤</span><b>Tus documentos</b><small>Facturas y albaranes a mano</small></button><button onClick={()=>onNavigate("orders")}><span>✓</span><b>Estado de pedidos</b><small>Sigue cada etapa y su fecha</small></button></section><section className="section panel"><div className="section-heading"><div><span className="eyebrow">MATERIAL PARA TU DÍA A DÍA</span><h2>Explora el catálogo</h2></div><button className="text-button" onClick={()=>onNavigate("catalog")}>Ver todo →</button></div><div className="products">{products.map(p=><ProductCard key={p.id} product={p} onAdd={onAdd}/>)}</div></section></div>
